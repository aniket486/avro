...
struct TestSchema {
    ...
    template <typename Serializer> void printUnion(Serializer& s, int path) {
        s.writeUnion(path);
        if (path == 0) {
    ...
    template <typename Serializer> void writeEncoding(Serializer& s, int path) {
        std::cout << "Record\n";
        s.writeRecord();
        s.writeInt(1000);
    ...
    template <typename Parser> void readMap(Parser& p) {
        int64_t size = 0;
        do {
            printNext(p);
    ...
    template <typename Parser> void readArray(Parser& p) {
        int64_t size = 0;
        double d = 0.0;
        do {
            printNext(p);
    ...
    template <typename Parser> void readNestedRecord(Parser& p) {
        printNext(p);
        p.readRecord();
        printNext(p);
        float f = p.readFloat();
    ...
    template <typename Parser> void readFixed(Parser& p) {
    ...
    template <typename Parser> void readData(Parser& p) {
        printNext(p);
        p.readRecord();
...
struct TestEncoding {
    ...
    template <typename IntType> void testEncoding(IntType start, IntType stop) {
    ...
    template <typename IntType> void testEncoding() {
...
struct TestNested {
    ...
    void createSchema() {
        ...
        arrayTree.addField(
            "children", ArraySchema(SymbolicSchema(Name("ArrayTree"), arrayTree.root())));
...
struct TestResolution {
    TestResolution()
        : int_(IntSchema()), long_(LongSchema()), bool_(BoolSchema()),
          float_(FloatSchema()), double_(DoubleSchema()),

          mapOfInt_(MapSchema(IntSchema())), mapOfDouble_(MapSchema(DoubleSchema())),
