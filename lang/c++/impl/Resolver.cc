...
#else
class NoOp {};
template <typename T> NoOp& operator<<(NoOp& noOp, const T&) { return noOp; }
NoOp noop;
#define DEBUG_OUT(str) noop << str
#endif

template <typename T> class PrimitiveSkipper : public Resolver {
  public:
...
template <typename T> class PrimitiveParser : public Resolver {
  public:
...
template <typename WT, typename RT> class PrimitivePromoter : public Resolver {
  public:
  ...
  private:
    ...
    template <typename T> void parseIt(Reader& reader, uint8_t* address) const {
...
template <> class PrimitiveSkipper<std::vector<uint8_t>> : public Resolver {
  public:
    PrimitiveSkipper() : Resolver() {}
...
template <> class PrimitiveParser<std::vector<uint8_t>> : public Resolver {
  public:
    PrimitiveParser(const PrimitiveLayout& offset)
        : Resolver(), offset_(offset.offset()) {}
...
class ResolverFactory : private boost::noncopyable {
    ...
    unique_ptr<Resolver> constructCompound(const NodePtr& writer, const NodePtr& reader,
        ...
        if (match == RESOLVE_NO_MATCH) {
            ...
            instruction = unique_ptr<Resolver>(
                new UnionToNonUnionParser(*this, writer, reader, offset));
        } else {
            const CompoundLayout& compoundLayout =
                dynamic_cast<const CompoundLayout&>(offset);
            instruction =
...
MapParser::MapParser(ResolverFactory& factory, const NodePtr& writer,
    ...
    : Resolver(),
      ...
      offset_(offsets.offset()), setFuncOffset_(offsets.at(0).offset()) {}
...
UnionParser::UnionParser(ResolverFactory& factory, const NodePtr& writer,
                         const NodePtr& reader, const CompoundLayout& offsets)
    : Resolver(), offset_(offsets.offset()), choiceOffset_(offsets.at(0).offset()),
...
NonUnionToUnionParser::NonUnionToUnionParser(ResolverFactory& factory,
    ...
    resolver_ =
        factory.construct(writer, reader->leafAt(choice_), offsets.at(choice_ + 2));
}
