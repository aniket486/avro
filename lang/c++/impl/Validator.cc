...
Validator::Validator(const ValidSchema& schema)
    : schema_(schema), nextType_(AVRO_NULL), expectedTypesFlag_(0),
      compoundStarted_(false), waitingForCount_(false), count_(0) {
...
bool Validator::getNextFieldName(std::string& name) const {
    ...
    name.clear();
    int idx =
        isCompound(nextType_) ? compoundStack_.size() - 2 : compoundStack_.size() - 1;
    if (idx >= 0 && compoundStack_[idx].node->type() == AVRO_RECORD) {
