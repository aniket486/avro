...
DataFileWriterBase::DataFileWriterBase(const char* filename, const ValidSchema& schema,
    ...
    : filename_(filename), schema_(schema), encoderPtr_(binaryEncoder()),
      syncInterval_(syncInterval), codec_(codec), stream_(fileOutputStream(filename)),
      buffer_(memoryOutputStream()), sync_(makeSync()), objectCount_(0) {
...
DataFileWriterBase::DataFileWriterBase(std::unique_ptr<OutputStream> outputStream,
                                       ...
                                       Codec codec)
    : filename_(NULL), schema_(schema), encoderPtr_(binaryEncoder()),
      syncInterval_(syncInterval), codec_(codec), stream_(std::move(outputStream)),
      buffer_(memoryOutputStream()), sync_(makeSync()), objectCount_(0) {
    init(schema, syncInterval, codec);
}
...
void DataFileWriterBase::sync() {
    ...
    if (codec_ == NULL_CODEC) {
        ...
        size_t compressed_size = snappy::Compress(
            reinterpret_cast<const char*>(temp.data()), temp.size(), &compressed);
        temp.clear();
        {
            boost::iostreams::filtering_ostream os;
            os.push(boost::iostreams::back_inserter(temp));
...
DataFileReaderBase::DataFileReaderBase(std::unique_ptr<InputStream> inputStream)
    : filename_(NULL), stream_(std::move(inputStream)), decoder_(binaryDecoder()),
      objectCount_(0), eof_(false) {
    readHeader();
}
...
void DataFileReaderBase::readDataBlock() {
    ...
    if (codec_ == NULL_CODEC) {
        ...
        uint32_t c = crc();
        if (checksum != c) {
            throw Exception(boost::format("Checksum did not match for Snappy "
                                          "compression: Expected: %1%, computed: %2%") %
                            checksum % c);
        }
